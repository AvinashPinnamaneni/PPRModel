...
def handle_orders(domains):
    '''
    step 1 : Get hold of orders
    step 2 : Get the list of products for each order and assign order id of each of the product object
        (The attribute order id will be generated dynamically for the product when the order is initiated)
    step 3 : Search for the process objects which has the products in the order as output product.
    step 4 : Search for the necessary input products of the process for the required output product
    '''
    for domain in domains:
        if domain.__name__ == 'Order':
            for order in domain.object_list: # iterating through each of the order
                print([products for products in order.products])
